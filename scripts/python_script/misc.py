...
def __valgrind (host, lock):
    ...
    '''
    client = get_ssh_client (host)
    ps_cmd = 'ps aux | grep -v grep | grep '
    if args.add == True:
        ps_cmd += args.process[0]
        stdin, stdout, stderr = client.exec_command ('ps aux|grep 29372')
        print ps_cmd, stdout
        #kill -9
        #valgrind
        ...
        print 'Error: invalid arguments!(REQUIRED: valgrind)\nExample: ' +\
              command_remainder['valgrind']
        exit (-1)
...
def valgrind ():
    ...
    if args.nodes == None or args.password == None \
            or (args.add == False and args.remove== False) \
            or (args.add == True and args.remove== True):
        print 'Error: invalid arguments!(REQUIRED: valgrind)\nExample: ' +\
              command_remainder['valgrind']
        exit (-1)

    __multi_thread (__valgrind)
...
if __name__ == '__main__':
    ...
    command_remainder['valgrind'] = sys_argv_0 + ' --valgrind --nodes 10.10.21.11{1,2,3} -p 9999 --password asdf --add/remove --process <PID/NAME>'
    ...
    #add or remove valgrind for ps aux|grep process
    parser.add_argument ('--valgrind', action='store_true',
                         help='add or delete valgrind for process')
    parser.add_argument ('--add', action='store_true', help='add valgrind')
    parser.add_argument ('--remove', action='store_true', help='delete valgrind')
    parser.add_argument ('--process', nargs=1, type=str,
                         help='the process_name or pid')
